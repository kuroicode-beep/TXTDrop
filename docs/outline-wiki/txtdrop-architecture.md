...
TXTDrop은 단일 Windows 데스크톱 프로세스 안에서 트레이, 전역 단축키, Tkinter UI, SQLite 저장소, Ollama·SVIL TTS·TXTAIMemory HTTP API를 연결한다. 앱 자체 백엔드는 없으며, Ollama는 로컬 서버 `http://localhost:11434`, SVIL TTS는 로컬 프록시 `http://127.0.0.1:8765`, TXTAIMemory는 control API `http://127.0.0.1:47530`를 사용한다. TTS 프록시가 꺼져 있으면 SVIL 웹 백엔드 `http://127.0.0.1:3000`을 통해 자동 기동을 요청한다(TXTAIMemory는 자동 기동하지 않음). 저장 폴더의 `.txt`는 연계 앱 TXTBrain이 수집한다.
...
flowchart TD
  ...
  User --> HotkeyMem["AI 기억 단축키<br/>Ctrl+Shift+M (옵트인)"]
  ...
  HotkeyMem --> Capture["capture_memory()<br/>daemon thread"]
  ...
  WebBackend -.기동.-> Tts
  Capture --> MemApi["TXTAIMemory control API<br/>127.0.0.1:47530<br/>/write"]
  Capture -.오프라인시.-> MemFallback["ai_memory_inbox/*.jsonl<br/>로컬 폴백"]
...
| daemon threads | `drop_clipboard`, `speak_clipboard`(TTS 자동 기동 폴링 포함), `capture_memory`, dedup 검사, Ollama 확인/제목 생성, 사운드 |
...
* SVIL TTS 연동은 `tts_client.py`가 `POST /api/tts/pipeline`을 호출한다 (`narration`, `play_async`). 서버 미기동 시 SVIL 웹 백엔드(`http://127.0.0.1:3000/api/local-server`, `action:start, id:tts`)로 자동 기동을 요청하고 최대 60초 헬스체크 폴링 후 재시도한다 (MCP `svil_tts_speak`과 동일 경로).
* TXTAIMemory 연동은 `memory_client.py`가 `POST http://127.0.0.1:47530/write`를 호출한다 (`source: "drop"`). **자동 기동은 하지 않는다** — 오프라인이면 `ai_memory_inbox/*.jsonl`에 로컬 폴백 저장한다. TXTAIMemory control API는 "localhost 전용, 인증 없음"으로 구현되어 있어 별도 페어링 절차가 없다.
...
* AI 기억 캡처는 자체 테이블 없이 `config`(설정)와 `log`(`memory` 카테고리, 감사 추적)만 사용한다. 오프라인 폴백은 DB가 아니라 저장 폴더 하위 `ai_memory_inbox/`의 JSON 라인 파일에 쌓인다.
...
* `/api/local-server`(TTS 자동 기동)와 `/write`(TXTAIMemory 캡처) 호출은 모두 인증이 필요 없는 로컬 전용 엔드포인트다.
...
* AI 기억 캡처는 TXTAIMemory를 자동 기동하지 않는다 — 오프라인이면 항상 파일 폴백으로 처리된다.

## 10. 보안 / 접근성

* 파일명은 Windows 금지 문자를 제거한다.
* DB 백업/복원은 사용자 선택 파일/폴더에 대해서만 수행한다.
* TTS 낭독 텍스트, AI 기억 캡처 텍스트 모두 외부 서버로 전송되지 않고 로컬 프록시(:8765)/control API(:47530)로만 전달된다.
* AI 기억 캡처는 기본 비활성(옵트인)이며, 사용자가 명시적으로 설정에서 켜야만 텍스트가 캡처된다.
