...
기본 설정에는 저장 폴더, 파일명 접두사, 사운드, 단축키, 언어, Ollama 모델, Ollama 자동 시작 여부가 포함된다.

## 05. 주요 훅 / 서비스

| 모듈 | 역할 |
|------|------|
| `main.py` | 단축키 등록, 클립보드 저장, 트레이 메뉴, Ollama 시작/새로고침 |
| `config.py` | SQLite 설정/로그/히스토리/백업/복원, 메모리 캐시 |
...
- 저장 성공/실패와 Ollama 상태는 커스텀 토스트로 표시한다 (5초, work area 기준 위치).
- Ollama 상태 확인과 제목 생성은 백그라운드 스레드에서 수행한다.
- 저장 성공 시 `sound_enabled=true`이면 `winsound.Beep`를 사용한다.

## 09. 주요 화면 스펙

- 트레이 우클릭 메뉴: 환경설정, 로그 기록, Ollama 새로고침, 종료
- 설정 창: 저장 폴더, 파일명 접두사, 단축키, Ollama, 사운드, 언어, DB 백업/복원
...
- 단축키 등록 실패 시 기본값 `ctrl+shift+z`로 재시도한다.

## 12. 검증 (2026-07-06)

- `py -3.12 -m compileall -q .` 통과
- PyInstaller onedir 빌드 성공 (`dist\TXTDrop\TXTDrop.exe`)
- Inno Setup 설치 파일 생성 성공 (`Output\TXTDropSetup.exe`)
- 첫 실행 데드락 수정 확인 (`_first_run` 직접 Tk 호출)
- Ollama 제목 생성 및 클립보드 첫 줄 폴백 동작 확인
